...
class TestKDTreePerformance:
    ...
    def test_kdtree_construction_performance(self, performance_data, perf_metrics):
        ...
        sklearn_tree, sklearn_time = time_function(
            SklearnKDTree, X, leaf_size=40
        )
        
        # Test numba construction
        numba_tree, numba_time = time_function(
        ...
        perf_metrics.record_metric(test_name, "construction_speedup", sklearn_time / numba_time)
        ...
        if n_samples >= 1000:  # Only assert on larger datasets where speedup is more likely
            assert numba_time < sklearn_time * 2.0, \
    ...
    def test_kdtree_query_performance_large_batch(self, performance_data, perf_metrics):
        ...
        perf_metrics.record_metric(test_name, "query_speedup", sklearn_time / numba_time)
        perf_metrics.record_metric(test_name, "queries_per_second_sklearn", n_queries / sklearn_time)
        perf_metrics.record_metric(test_name, "queries_per_second_numba", n_queries / numba_time)
        ...
        if n_queries >= 3000:  # Only assert performance for large enough batches where advantage is consistent
            assert numba_time < sklearn_time * 1.5, \
            ...
            assert sklearn_time / numba_time > 1.5, \
            ...
            assert sklearn_time / numba_time > 1.0, \
            ...
            assert numba_time < sklearn_time * 4.0, \
                f"Numba queries excessively slow for batch ({n_queries} queries): {numba_time:.4f}s vs sklearn {sklearn_time:.4f}s"
    
    def test_kdtree_query_performance_massive_batch(self, performance_data, perf_metrics):
        ...
        np.random.seed(124)
        n_queries = max(5000, n_samples)  # Very large batch - equal or larger than training set
        query_data = np.random.rand(n_queries, n_features).astype(np.float32) * 10.0
        ...
        perf_metrics.record_metric(test_name, "batch_speedup", sklearn_time / numba_time)
        perf_metrics.record_metric(test_name, "batch_queries_per_second_sklearn", n_queries / sklearn_time)
        perf_metrics.record_metric(test_name, "batch_queries_per_second_numba", n_queries / numba_time)
        ...
        assert numba_time < sklearn_time * 1.2, \
        ...
        assert sklearn_time / numba_time > 2.0, \
    ...
    def test_kdtree_accuracy_comparison(self, performance_data, perf_metrics):
        ...
        np.random.seed(125)
        query_indices = np.random.choice(n_samples, size=min(50, n_samples), replace=False)
        ...
        print(f"  Mean distance difference: {np.mean(first_neighbor_distance_diff):.6f}")
        
        # Allow small numerical differences
        assert max_distance_diff < 1e-5, \
        ...
        assert match_rate > 0.95, f"Nearest neighbor agreement too low: {match_rate:.2%}"
    ...
    def test_kdtree_scaling_performance(self, perf_metrics):
        ...
        for i in range(1, len(sizes)):
            ...
            assert sklearn_time_ratio < max_expected_ratio * 2, \
                f"Sklearn scaling too poor: {sklearn_time_ratio:.2f}x for {size_ratio:.2f}x data"
            assert numba_time_ratio < max_expected_ratio * 2, \
    ...
    def test_kdtree_different_k_values(self, perf_metrics):
        ...
        for k in k_values:
            ...
            assert numba_time < sklearn_time * 3.0, \
    ...
    def test_kdtree_query_batch_scaling(self, perf_metrics):
        ...
        for batch_size in batch_sizes:
            ...
            query_data = np.random.rand(batch_size, n_features).astype(np.float32) * 10.0
        ...
        print(f"  Numba shows increasing advantage with larger batches due to parallelization benefits")
        print(f"  Small batches (<1000) have overhead, large batches (>2000) show significant speedup")
    ...
    def test_kdtree_query_performance_ultra_large_batch(self, perf_metrics):
        ...
        print(f"  Efficiency gain: {(numba_qps - sklearn_qps):,.0f} additional queries/sec")
        ...
        assert numba_time < sklearn_time, \
            f"Numba should be faster for ultra-large batches: {speedup:.2f}x"
        
        # Expect substantial speedup on ultra-large batches
        assert speedup > 3.0, \
            f"Expected major numba advantage for ultra-large batches: {speedup:.2f}x (target: >3x)"
        
        # Throughput should be significantly higher
        assert numba_qps > sklearn_qps * 3.0, \
            f"Expected 3x+ throughput improvement: {numba_qps/sklearn_qps:.2f}x"
...
class TestKDTreeRegressionBaseline:
    ...
    def test_kdtree_baseline_performance(self):
        ...
        assert numba_construction_time < 2.0, \
            f"Construction too slow: {numba_construction_time:.4f}s"
        assert numba_query_time < 1.0, \
            f"Queries too slow: {numba_query_time:.4f}s"
        ...
        if construction_speedup > 1.0:
            ...
            print(f"  ⚠️  Construction {1/construction_speedup:.2f}x slower than sklearn")
