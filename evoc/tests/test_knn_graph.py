...
class TestUtilityFunctions:
    ...
    def test_ts_returns_string(self):
        ...
        assert any(month in timestamp for month in ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
...
class TestMakeForest:
    ...
    def test_make_forest_float32(self, float_data):
        ...
        result = make_forest(
            float_data, n_neighbors, n_trees, leaf_size,
            random_state, np.float32
        )
    ...
    def test_make_forest_uint8(self, uint8_data):
        ...
        result = make_forest(
            uint8_data, n_neighbors, n_trees, leaf_size,
            random_state, np.uint8
        )
    ...
    def test_make_forest_int8(self, int8_data):
        ...
        result = make_forest(
            int8_data, n_neighbors, n_trees, leaf_size,
            random_state, np.int8
        )
    ...
    def test_make_forest_default_leaf_size(self, float_data):
        ...
        result = make_forest(
            float_data, n_neighbors, n_trees, None,
            random_state, np.float32
        )
    ...
    def test_make_forest_different_max_depth(self, float_data):
        ...
        result_shallow = make_forest(
            float_data, n_neighbors, n_trees, leaf_size,
            random_state, np.float32, max_depth=5
        )
        ...
        result_deep = make_forest(
            float_data, n_neighbors, n_trees, leaf_size,
            random_state, np.float32, max_depth=500
        )
    ...
    def test_make_forest_exception_handling(self, mock_make_float_forest, float_data):
        ...
        with pytest.warns(UserWarning, match="Random Projection forest initialisation failed"):
            result = make_forest(
                float_data, 10, 4, 20,
                random_state, np.float32
            )
...
class TestNNDescent:
    ...
    def test_nn_descent_float32(self, float_data):
        ...
        with patch('evoc.knn_graph.nn_descent_float') as mock_nn_descent:
            ...
            mock_indices = np.random.randint(0, len(float_data), 
                                           size=(len(float_data), n_neighbors))
            mock_distances = -np.random.exponential(1, size=(len(float_data), n_neighbors))
            mock_nn_descent.return_value = (mock_indices, mock_distances)
            
            result = nn_descent(
                float_data, n_neighbors, rng_state, 30, 5, 0.001,
    ...
    def test_nn_descent_uint8(self, uint8_data):
        ...
        with patch('evoc.knn_graph.nn_descent_uint8') as mock_nn_descent:
            mock_indices = np.random.randint(0, len(uint8_data), 
                                           size=(len(uint8_data), n_neighbors))
            mock_distances = -np.random.exponential(1, size=(len(uint8_data), n_neighbors))
            mock_nn_descent.return_value = (mock_indices, mock_distances)
            
            result = nn_descent(
                uint8_data, n_neighbors, rng_state, 30, 5, 0.001,
                np.uint8, verbose=True
            )
    ...
    def test_nn_descent_int8(self, int8_data):
        ...
        with patch('evoc.knn_graph.nn_descent_int8') as mock_nn_descent:
            mock_indices = np.random.randint(0, len(int8_data), 
                                           size=(len(int8_data), n_neighbors))
            mock_distances = -np.random.exponential(1, size=(len(int8_data), n_neighbors))
            mock_nn_descent.return_value = (mock_indices, mock_distances)
            
            result = nn_descent(
                int8_data, n_neighbors, rng_state, 30, 5, 0.001,
                np.int8
            )
...
class TestKNNGraph:
    ...
    def float_test_data(self):
        ...
        X, y = make_blobs(n_samples=200, centers=4, n_features=50, 
    ...
    def test_knn_graph_n_jobs_setting(self, float_test_data):
        ...
        with patch('numba.get_num_threads') as mock_get_threads, \
    ...
    def test_knn_graph_auto_parameters(self, float_test_data):
        ...
        with patch('numba.get_num_threads', return_value=12):
            result = knn_graph(
                float_test_data,
    ...
    def test_knn_graph_warning_on_failure(self, float_test_data):
        ...
        with patch('evoc.knn_graph.nn_descent') as mock_nn_descent:
            ...
            with pytest.warns(UserWarning, match="Failed to correctly find n_neighbors"):
...
class TestIntegration:
    ...
    def test_small_dataset_complete_pipeline(self):
        ...
        X, y = make_blobs(n_samples=50, centers=3, n_features=10, 
