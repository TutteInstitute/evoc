...
class TestKNNGraphPerformance:
    ...
    def test_knn_graph_scaling_performance(self, performance_data, perf_metrics):
        ...
        perf_metrics.record_metric(test_name, "samples_per_second", n_samples / duration)
        ...
        assert samples_per_sec > min_expected, \
    ...
    def test_knn_graph_parameter_performance(self, perf_metrics):
        ...
        for config in param_configs:
            ...
            # Warm up
            knn_graph(X[:100], n_neighbors=config["n_neighbors"], 
                     n_trees=config["n_trees"], random_state=42)
            
            # Timed run
            result, duration = time_function(
                knn_graph, X, random_state=42, **config
            )
    ...
    def test_knn_graph_threading_performance(self, perf_metrics):
        ...
        if HAS_PSUTIL:
            max_threads = min(8, psutil.cpu_count(logical=True))  # Don't exceed available cores
        else:
            import os
        ...
        for n_jobs in thread_counts:
            ...
            # Warm up
            knn_graph(X[:100], n_neighbors=10, n_trees=2, n_jobs=n_jobs, random_state=42)
        ...
        if len(durations) >= 2 and max_threads > 1:
            ...
            if max_threads > 2:
                assert speedup_ratio > expected_min_speedup, \
    ...
    def test_memory_usage_scaling(self, perf_metrics):
        ...
        for n_samples, n_features in test_sizes:
            ...
            result = knn_graph(X, n_neighbors=20, n_trees=4, random_state=42, verbose=False)
        ...
        if len(memory_usages) >= 2:
            ...
            assert memory_ratio < sample_ratio ** 1.5, \
    ...
    def test_reproducibility_performance(self, perf_metrics):
        ...
        assert cv < 0.2, f"Performance too variable: CV = {cv:.3f}"
